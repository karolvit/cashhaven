...
let ws;
let pingInterval;
const processedPedidos = new Set();
function connectWebSocket() {
    console.log('🧩 Chamando connectWebSocket...');
    if (ws && (ws.readyState === ws_1.WebSocket.OPEN || ws.readyState === ws_1.WebSocket.CONNECTING)) {
    ...
    ws = new ws_1.WebSocket('ws://celebreserver2.ddns.net:5238/');
    ws.on('open', () => {
    ...
    ws.on('message', (data) => {
        try {
            ...
            const pedidoArray = message.pedido;
            const printerIp = `${message.imp}`;
            const printerPort = 9100;
            ping.sys.probe(printerIp, (isAlive) => {
                if (!isAlive) {
